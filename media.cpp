...
static UniqueAVCodecContext init_ffv1_context(int universes) {
    ...
    if (!ffv1)
        throw std::runtime_error{"finding FFV1 encoder"};

    auto *ffv1_ctx = avcodec_alloc_context3(nullptr);
    if (!ffv1_ctx)
        throw std::runtime_error{"allocating encoder context"};
...
static UniqueAVFormatContext init_mkv_context() {
    ...
    if (!fmt)
        throw std::runtime_error{"finding MKV muxer"};
...
DMXVideoEncoder::DMXVideoEncoder(int universes, const std::string& path)
    :enc_ctx{init_ffv1_context(universes)}, fmt_ctx{init_mkv_context()},
     io_ctx{init_output_context(path)}, fbuf{init_frame(enc_ctx.get())} {
    
    fmt_ctx->pb = io_ctx.get();

    s = avformat_new_stream(fmt_ctx.get(), enc_ctx->codec);
    if (!s)
        throw std::runtime_error{"allocating stream for muxer"};
...
DMXVideoEncoder::~DMXVideoEncoder() {
    close();
}

void DMXVideoEncoder::ensure_not_closed() {
    if (closed)
        throw std::logic_error{"closed"};
}
