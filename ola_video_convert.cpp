...
int prog(int argc, char **argv) {
    ...
    while (read_frame(show, d_frame) || (d_frame.duration_ms == -1)) {
        ...
        if (!d_frame.duration_ms)
            continue;

        if (universe_states.size() != num_universe)
            throw std::runtime_error{
                "universe state(s) undefined at encode"};
        
        encoder.write_universe(universe_states, 
            std::max(d_frame.duration_ms, static_cast<int64_t>(1)));
    };

    if (!show.eof())
        throw std::runtime_error{"reading showfile"};
